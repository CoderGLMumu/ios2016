...
@interface NewWisdomListVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation NewWisdomListVC

static NSString *ID = @"NewWisdomCell";
...
- (void)loadData
{
    ...
    NSArray *arrT = [NSKeyedUnarchiver unarchiveObjectWithFile:CachesNotificationNew_M_Wisdom];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!appD.vip) {
        [UIView bch_showWithTitle:@"提示" message:@"查看智库详情要先加入建众帮会员哦！" buttonTitles:@[@"取消",@"确定"] callback:^(id sender, NSUInteger buttonIndex) {
            if (1 == buttonIndex) {
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [UIView bch_showWithTitle:@"确认删除" message:nil buttonTitles:@[@"取消",@"删除"] callback:^(id sender, NSUInteger buttonIndex) {
        if (buttonIndex == 0) {
            ...
            NSMutableArray *arrT = [NSKeyedUnarchiver unarchiveObjectWithFile:CachesNotificationNew_M_Wisdom];
            ...
            [NSKeyedArchiver archiveRootObject:arrT toFile:CachesNotificationNew_M_Wisdom];
