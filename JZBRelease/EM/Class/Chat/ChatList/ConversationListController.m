...
/** LoginID */
@property (nonatomic, strong) NSString *PreLoginID;
...
//    self.tableView.frame = CGRectMake(0, -15, self.view.frame.size.width, self.view.frame.size.height - self.searchBar.frame.size.height);
...
{
    if ([self.PreLoginID isEqualToString:[LoginVM getInstance].readLocal._id]) {
        
    }else {
        [self refreshDataSource];
    }
    
    self.PreLoginID = [LoginVM getInstance].readLocal._id;
                                    ...
                                    modelForConversation:(EMConversation *)conversation
{
    ...
    if (model.title.length > 7){
        NSString *temp_str = [model.title substringFromIndex:7];
        
        NSString *path = [NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject];
        NSString *filename = [path stringByAppendingPathComponent:@"contactsSource.plist"];
        NSArray *arr = [NSArray arrayWithContentsOfFile:filename];
        
        for (NSDictionary *dictM in arr) {
            if ([temp_str isEqualToString:dictM.allKeys[0]]){
                model.title = dictM.allValues[0];
            }
        }
    }
