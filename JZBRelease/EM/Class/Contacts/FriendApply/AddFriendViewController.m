...
//        NSString *str = [[access_token substringFromIndex:1]substringToIndex:1];
...
//        NSLog(@"%@temp_str",buddyName);
        
         [self HandleNetWork:parameters :buddyName :message];
    }
}

- (void)HandleNetWork:(NSDictionary *)parameters :(NSString *)buddyName :(NSString *)message
{
    ...
    [HttpToolSDK postWithURL:[[ValuesFromXML getValueWithName:abPath WithKind:XMLTypeNetPort] stringByAppendingPathComponent:@"Web/Friend/add"] parameters:parameters success:^(id json) {
        ...
        if ([json[@"error"][@"errcode"] isEqual:@(501)]) {
            BOOL ttttt = [[LoginVM getInstance] loginWithUserInfoForGetNewToken:[[LoginVM getInstance] readLocal]];
            NSLog(@"%d",ttttt);
            if (!isLock) {
                [self HandleNetWork:parameters :buddyName :message];
                isLock = YES;
        ...
        /** 添加好友操作 */
        EMError *error = [[EMClient sharedClient].contactManager addContact:[NSString stringWithFormat:@"member_%@",buddyName] message:message];
