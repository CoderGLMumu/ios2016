...
- (void)applyCellAddFriendAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row < [self.dataSource count]) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            else if(applyStyle == ApplyStyleFriend){ // 添加单个好友
...
//                NSString *access_token = [defaults stringForKey:@"access_token"];
                ...
                if (entity.applicantUsername.length > 7){
                    NSString *temp_str = [entity.applicantUsername substringFromIndex:7];
...
//                    NSLog(@"%@temp_str",temp_str);
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (!error) {
                    ...
                    if ([error.errorDescription containsString:@"not exist"]) {
                        [self showHint:@"想加入的内容已经不存在"];
                        [self.dataSource removeObject:entity];
                        [self.tableView reloadData];
                    }else {
                        [self showHint:NSLocalizedString(@"acceptFail", @"accept failure")];
