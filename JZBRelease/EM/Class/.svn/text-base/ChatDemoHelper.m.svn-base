...
- (void)didReceiveMessages:(NSArray *)aMessages
{
    ...
    for(EMMessage *message in aMessages){
        ...
        if (_chatVC == nil || !isChatting) {
            ...
            if (self.mainVC) {
                [_mainVC setupUnreadMessageCount]; //1
            }
            return;
        }
        ...
        if (aReason != EMCallEndReasonHangup) {
            ...
            if (aError) {
                ...
                if (aError.code == EMErrorCallRemoteOffline) {
                    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"对方不在线" delegate:nil cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
                    [alertView show];
                }
