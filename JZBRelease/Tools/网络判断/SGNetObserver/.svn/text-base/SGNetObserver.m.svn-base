...
//    obsever.host = @"www.baidu.com";
    obsever.host = [ValuesFromXML getValueWithName:@"Sever_Absolute_Address" WithKind:XMLTypeNetPort];
    return obsever;
}
...
- (void)networkStatusDidChanged{
    ...
    if (!status1) {
        [UIView bch_showWithTitle:@"已为“建众帮”关闭蜂窝移动数据" message:@"您可以在“设置”中为本应用程序打开蜂窝移动数据" buttonTitles:@[@"设置",@"好"] callback:^(id sender, NSUInteger buttonIndex) {
