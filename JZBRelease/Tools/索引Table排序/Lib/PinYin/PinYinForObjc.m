...
//    NSString *outputPinyin = [PinyinHelper toHanyuPinyinStringWithNSString:sourceText withHanyuPinyinOutputFormat:outputFormat withNSString:@""];
     NSString *outputPinyin = [PinyinHelper toHanyuPinyinStringWithNSString2:sourceText withHanyuPinyinOutputFormat:outputFormat withNSString:@""];
...
+ (NSString*)chineseConvertToPinYinHead:(NSString *)chinese {
    ...
    if ([chinese isKindOfClass:[RecommendPersonItem class]]) {
        
        RecommendPersonItem *item = (RecommendPersonItem *)chinese;
        
        for (int i=0;i <item.nickname.length;i++) {
            NSString *mainPinyinStrOfChar = [PinyinHelper getFirstHanyuPinyinStringWithChar:[item.nickname characterAtIndex:i] withHanyuPinyinOutputFormat:outputFormat];
        ...
        if (outputPinyin.length==0) {
            outputPinyin = (NSMutableString*)[self englishToHeade:item.nickname];
        }
