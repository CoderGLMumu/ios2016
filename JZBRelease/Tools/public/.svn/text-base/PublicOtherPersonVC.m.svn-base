...
//    if ([self.user.uid isEqualToString:[LoginVM getInstance].readLocal._id]) {
...
//    }
    ...
    btn2.backgroundColor = [UIColor hx_colorWithHexRGBAString:@"ff9800"];
//    btn2.backgroundColor = [UIColor hx_colorWithHexRGBAString:@"fb514d"];
...
//    [self.userAvaImageView sd_setImageWithURL:[NSURL URLWithString:[[ValuesFromXML getValueWithName:@"Images_Absolute_Address" WithKind:XMLTypeNetPort] stringByAppendingPathComponent:self.user.avatar]] placeholderImage:[UIImage imageNamed:@"ZCCG_TX"]];
    ...
    [LocalDataRW getImageWithDirectory:Directory_RM RetalivePath:[[ValuesFromXML getValueWithName:@"Images_Absolute_Address" WithKind:XMLTypeNetPort] stringByAppendingPathComponent:self.user.avatar] WithContainerImageView:self.userAvaImageView];
...
- (void)MesFriend
{
    if ([self.navigationController isKindOfClass:[GLNAVC class]]) {
        ...
        chatViewController.ispresentVC = YES;
        
        GLNAVC * jackNavigationController = [[GLNAVC alloc] initWithRootViewController:chatViewController];
        
        [self presentViewController:jackNavigationController animated:YES completion:nil];
        
//        self.navigationController = jackNavigationController;
//        [self.navigationController pushViewController:jackNavigationController animated:YES];
    }
