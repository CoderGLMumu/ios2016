...
- (void)viewDidLoad {
    ...
    unFocusAry = [LocalDataRW readDataFromLocalOfDocument:@"unFocusTags.plist" WithDirectory_Type:Directory_BB];
    [self downloadData];
//    if (!unFocusAry) {
//        
//    }else{
        ...
        [[HttpManager shareManager] netStatus:^(AFNetworkReachabilityStatus status) {
            if(status == AFNetworkReachabilityStatusReachableViaWWAN || status ==AFNetworkReachabilityStatusReachableViaWiFi){
                ...
                send.returnArray = ^(NSArray *ary){
                    if (ary && ary.count > 0) {
                        if ([LocalDataRW writeDataToLocaOfDocument:ary WithDirectory_Type:Directory_BB AtFileName:@"AllTags.plist"]) {
                        ...
                        for (NSInteger i = 0; i < ary.count; i ++) {
                            ...
                            for (int j = 0; j < self.focusedAry.count; j ++) {
                                ...
                                if ([industryModel.name isEqualToString:tag]) {
                                    isExsist = YES;
                                    break;
                                }
                            }
                            if (!isExsist) {
                                [unFocusAry addObject:industryModel.name];
                            }

                        }
                        [LocalDataRW writeDataToLocaOfDocument:unFocusAry WithDirectory_Type:Directory_BB AtFileName:@"unFocusTags.plist"];
...
-(void) backAction{
    ...
    [LocalDataRW writeDataToLocaOfDocument:tagList.tagArray WithDirectory_Type:Directory_BB AtFileName:@"focusedTags.plist"];
    [LocalDataRW writeDataToLocaOfDocument:tagList1.tagArray WithDirectory_Type:Directory_BB AtFileName:@"unFocusTags.plist"];
