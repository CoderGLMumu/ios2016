...
//    self.timer = [NSTimer timerWithTimeInterval:10.0 target:self selector:@selector(taskForMediaType_LIVE) userInfo:nil repeats:YES];
...
//        if (mPlayer.mediaType != MediaType_VOD) {
//            
////            [[NSRunLoop currentRunLoop] addTimer:self.timer forMode:NSRunLoopCommonModes];
//        }
//    });
...
- (void)taskForMediaType_LIVE
{
    GLLog(@"taskForMediaType_LIVE")
//    [self showLoadingIndicators];
...
//            error_msg = @"网络连接失败";
            if (mPlayer.mediaType == MediaType_LIVE) {
                error_msg = @"亲，直播已经结束 或 您的网络不流畅。";
            }else {
                error_msg = @"亲，您的网络不流畅";
            }
            
//            error_msg = @"亲，您的网络不流畅，或直播已经结束。";
...
//                                              otherButtonTitles:@"重新连接",nil];
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"直播提示"
                                                        message:error_msg
                                                       delegate:self
...
//        [mPlayer play];
        ...
        if (self.callBack) {
            self.callBack(@"Close",VideoActionTypeClose);
        }
...
- (void)OnVideoFinish:(NSNotification *)notification {
    ...
    UIAlertView *alter = [[UIAlertView alloc] initWithTitle:@"播放完成" message:@"播放完成" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil];
...
- (void)OnStartCache:(NSNotification *)notification {
    ...
    GLLog(@"OnStartCache")
    GLLog(@"OnStartCache")
    GLLog(@"%ldmPlayer.mediaType",(long)mPlayer.mediaType)
    
//    if (mPlayer.mediaType == MediaType_LIVE) {
//        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"直播提示"
//                                                        message:@"亲，直播结束了，或者你网络太差请重试"
//                                                       delegate:self
//                                              cancelButtonTitle:@"关闭"
//                                              otherButtonTitles:@"重新连接",@"等待",nil];
//        [alert show];
//    }else {
//        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"直播提示"
//                                                        message:@"你网络太差请重试"
