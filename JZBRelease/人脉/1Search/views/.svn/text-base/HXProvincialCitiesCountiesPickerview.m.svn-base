...
- (void)internalConfig {
    ...
    button.frame = CGRectMake(self.frame.size.width - 57, 0, 47, 43.5);
    button.backgroundColor = [UIColor clearColor];
    button.titleLabel.font = [UIFont systemFontOfSize:15];
    [button setTitleColor:[UIColor colorWithRed:0/255.0 green:122/255.0 blue:255/255.0 alpha:1.0] forState:UIControlStateNormal];
...
//            }
            if ([provinceDic[@"value"][@"name"] isEqualToString:provinceName]) {
                _firstCurrentIndex = idx+1;
                _secondAry = provinceDic[@"son"];
            }
        }];
    } else {
