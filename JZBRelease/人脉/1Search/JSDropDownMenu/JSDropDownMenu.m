...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height {
    ...
    self = [self initWithFrame:CGRectMake(origin.x, origin.y, 55, height)];
    if (self) {
        _origin = origin;
        ...
        _backGroundView = [[UIView alloc] initWithFrame:CGRectMake(0, origin.y, screenSize.width, screenSize.height)];
...
- (void)animateCollectionView:(UICollectionView *)collectionView show:(BOOL)show complete:(void(^)())complete {
    ...
    if (show) {
        ...
        if (collectionView) {
            ...
            collectionView.frame = CGRectMake(_origin.x, self.frame.origin.y + self.frame.size.height, [UIScreen mainScreen].bounds.size.width, 0);
        ...
        [UIView animateWithDuration:0.2 animations:^{
            if (collectionView) {
                collectionView.frame = CGRectMake(0, self.frame.origin.y + self.frame.size.height, [UIScreen mainScreen].bounds.size.width, collectionViewHeight);
            }
        }];
    } else {
        [UIView animateWithDuration:0.2 animations:^{
